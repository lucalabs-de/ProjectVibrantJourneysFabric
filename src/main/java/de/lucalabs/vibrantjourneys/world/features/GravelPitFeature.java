...
public class GravelPitFeature extends Feature<NoneFeatureConfiguration> {
                    ...
                    for (int l3 = 0; l3 < 8; l3++) {
                        ...
                        if (flag2 && (l3 < 4 || randomsource.nextInt(2) != 0)) {
                            ...
                            if (blockstate.isSolid() && !blockstate.is(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE)) {
                                ...
                                BlockState block;
                                if (worldgenlevel.getBlockState(blockpos3.up()).canBeReplaced() && randomsource.nextFloat() < 0.17F) {
    ...
    private BlockState getGravelBlock(Random random) {
        ...
        if (chance < 0.02F) {
            return PVJBlocks.GILDED_GRAVEL.getDefaultState();
        } else if (chance < 0.07F) {
            return PVJBlocks.FERROUS_GRAVEL.getDefaultState();
        }

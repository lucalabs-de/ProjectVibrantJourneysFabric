...
public class WarpedForestRuinedPortalDecorator extends RuinedPortalDecoratorBase {
    ...
    public void decorate(StructureWorldAccess level, ChunkGenerator generator, Random random, BlockPos groundPos) {
        ...
        if (WorldUtils.isEmptyOrReplaceable(level, pos)) {
            float chance = random.nextFloat();
            if (chance < 0.02F) {
                Optional<? extends RegistryEntry<ConfiguredFeature<?, ?>>> bigMushroom = level.getRegistryManager()
                        .get(RegistryKeys.CONFIGURED_FEATURE)
                        .getEntry(TreeConfiguredFeatures.WARPED_FUNGUS);
                bigMushroom.ifPresent((feature) -> feature.value().generate(level, generator, random, pos));
            } else if (chance < 0.15F) {
                Optional<? extends RegistryEntry<ConfiguredFeature<?, ?>>> warpedForestVegetation = level.getRegistryManager()
                        .get(RegistryKeys.CONFIGURED_FEATURE)
                        .getEntry(NetherConfiguredFeatures.WARPED_FOREST_VEGETATION);
                warpedForestVegetation.ifPresent((feature) -> feature.value().generate(level, generator, random, pos));
            } else if (chance < 0.16F) {
                Optional<? extends RegistryEntry<ConfiguredFeature<?, ?>>> twistingVines = level.getRegistryManager()
                        .get(RegistryKeys.CONFIGURED_FEATURE)
                        .getEntry(NetherConfiguredFeatures.TWISTING_VINES_BONEMEAL);
